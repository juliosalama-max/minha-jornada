...
test("renders the manifest with the per-app name", () => {
  const manifest = JSON.parse(renderWebManifest("wild-race.grok.me", { site: {} }));
  assert.equal(manifest.name, "Wild Race");
  assert.equal(manifest.short_name, "Wild Race");
  assert.equal(manifest.icons[0].src, "/__grok/icon-180.png");
});

test("custom domain uses site title and custom icons", () => {
  const manifest = JSON.parse(
    renderWebManifest("minha-jornada.alinelucena.com.br", {
      site: {
        title: "Minha Jornada",
        short_name: "Jornada",
        color: "1B7A72",
  ...
  assert.equal(manifest.name, "Minha Jornada");
  assert.equal(manifest.short_name, "Jornada");
  assert.equal(manifest.theme_color, "#1B7A72");
